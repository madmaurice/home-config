...
"
       :let Fgrep_Path = 'C:\\cygwin\\bin\\grep -F'
"
...
"
       :let Egrep_Path = 'C:\\cygwin\\bin\\grep -E'
"
...
function! s:RunGrepRecursive(cmd_name, grep_cmd, action, ...)
    ...
    if g:Grep_Find_Use_Xargs == 1
        " strot quoting startdir let cmd = g:Grep_Find_Path . " " . startdir
...
" strot disabled as it disallows using :E for :Explore
...
" strot added
" iterate thru quickfix results (ex for Grep plugin)
map <C-Up>   :cprev<cr>
map <C-Down> :cnext<cr>
